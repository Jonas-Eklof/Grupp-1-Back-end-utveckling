...
router.get("/cart/:user_id", (req, res) => {
  ...
  try {
    const stmt = db.prepare(`
      SELECT Cart.cart_id, Products.name, Cart.quantity, Products.price
      FROM Cart
...
router.delete("/cart/:cart_id", (req, res) => {
  ...
  try {
    const stmt = db.prepare("DELETE FROM Cart WHERE cart_id = ?");
...
router.delete("/cart/user/:user_id", (req, res) => {
  const { user_id } = req.params;
...
router.post("/orders", (req, res) => {
  ...
  try {
    ...
    // 4. Flytta produkter från Cart till Order_Items
